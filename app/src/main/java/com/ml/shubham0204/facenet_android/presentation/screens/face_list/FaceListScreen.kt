...
@Composable
fun FaceListScreen(onNavigateBack: (() -> Unit),
                   onAddFaceClick: (() -> Unit),
    ...
    FaceNetAndroidTheme {
        ...
        var sliderValue by remember { mutableFloatStateOf(threshold) }
        Scaffold(
            ...
            topBar = {
                TopAppBar(
                    ...
                    actions = {
                        IconButton(onClick = onExportLogClick) {
                            Icon(
                                imageVector = Icons.Default.FileDownload,
                                contentDescription = "Export Log"
                            )
                        }
                        IconButton(onClick = onClearLogClick) {
                            Icon(
                                imageVector = Icons.Default.DeleteForever,
                                contentDescription = "Delete Log"
                            )
                        }
                        IconButton(onClick = onBatchAddClick) {
                            Icon(
                        ...
                        IconButton(onClick = { showDialog = true }) {
                            Icon(Icons.Default.Settings, contentDescription = "Set Threshold")
                        }
                    }
                )
            },
        ...
        if (showDialog) {
            AlertDialog(
                ...
                title = { Text("Set Recognition Threshold") },
                text = {
                    Column {
                        Text("Current: %.2f".format(sliderValue))
                        Slider(
                            value = sliderValue,
                ...
                confirmButton = {
                    Button(onClick = {
                        viewModel.setThreshold(sliderValue)
                        showDialog = false
                    }) {
                        Text("Save")
                    }
